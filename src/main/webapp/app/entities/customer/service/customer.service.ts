...
export class CustomerService {
  ...
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/_search/customers');
  ...
  update(customer: ICustomer): Observable<EntityResponseType> {
    return this.http.put<ICustomer>(`${this.resourceUrl}/${getCustomerIdentifier(customer) as string}`, customer, { observe: 'response' });
  }

  partialUpdate(customer: ICustomer): Observable<EntityResponseType> {
    return this.http.patch<ICustomer>(`${this.resourceUrl}/${getCustomerIdentifier(customer) as string}`, customer, {
      observe: 'response',
    });
  }
