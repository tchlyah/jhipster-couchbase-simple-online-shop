...
echo "Setup services"
curl http://127.0.0.1:8091/node/controller/setupServices -d services=kv%2Cn1ql%2Cindex%2Cfts%2Ceventing
...
curl -u Administrator:password -X POST http://127.0.0.1:8091/pools/default/buckets -d name=$BUCKET -d bucketType=couchbase -d ramQuotaMB=600 -dauthType=sasl
...
if [ "$TYPE" = "WORKER" ]; then
    ...
    if [ "$AUTO_REBALANCE" = "true" ]; then
        ...
        couchbase-cli server-add --cluster=$COUCHBASE_MASTER:8091 --user=Administrator --password=password --server-add=$IP --server-add-username=Administrator --server-add-password=password --services=data,index,query,fts
    fi;
fi;
